...
function deleteState() {
	...
	url = CONTEXT_PATH + "states/delete/" + stateId;
...
function loadStates4Country() {
	selectedCountry = $("#dropDownCountriesForStates option:selected");
	countryId = selectedCountry.val();
	url = CONTEXT_PATH + "states/list_by_country/" + countryId;

	$.get(url, function(responseJSON) {
		dropDownStates.empty();
		//alert(responseJSON);
		$.each(responseJSON, function(index, state) {
			//alert($("<option>").val(state.id).text(state.name).appendTo(dropDownStates));
...
function loadCountries4States() {
	url = CONTEXT_PATH + "countries/list";
	$.get(url, function(responseJSON) {
